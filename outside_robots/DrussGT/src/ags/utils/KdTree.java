...
public abstract class KdTree<T> {
    ...
    //NB!! don't mix this with removeOld!
    private void removeNearest(double[] nearLocation) {
        double[] location = nearestNeighbor(nearLocation, 1, false).get(0).location;
    ...
    public static class Entry<T> {
        ...
        private Entry(double[] location, double distance, T value) {
    ...
    public List<Entry<T>> nearestNeighbor(double[] location, int count, boolean sequentialSorting) {
            ...
            if (cursor.status == Status.NONE && cursor.locations != null) {
                ...
                if (cursor.locationCount > 0) {
                    if (cursor.singularity) {
                        ...
                        if (dist <= range) {
                            for (int i = 0; i < cursor.locationCount; i++) {
                                resultHeap.addValue(cursor.locations[i], dist, cursor.data[i]);
                            }
                        }
                    } else {
                        for (int i = 0; i < cursor.locationCount; i++) {
        ...
        if (sequentialSorting) {
            while (resultHeap.values > 0) {
                ...
                results.add(new Entry<T>(resultHeap.removedLocation, resultHeap.removedDist, (T) resultHeap.removedData));
            }
        } else {
            for (int i = 0; i < resultHeap.values; i++) {
                results.add(new Entry<T>(resultHeap.location[i], resultHeap.distance[i], (T) resultHeap.data[i]));
            }
        }
    ...
    private class ResultHeap {
        ...
        public double[] removedLocation;

        public ResultHeap(int size) {
            this.data = new Object[size];
            this.distance = new double[size];
            this.location = new double[size][dimensions];
            this.size = size;
            this.values = 0;
        }

        public void addValue(double[] loc, double dist, Object value) {
        ...
        public void removeLargest() {
            ...
            removedLocation = location[0];
            values--;
            data[0] = data[values];
            distance[0] = distance[values];
            location[0] = location[values];
            downHeapify(0);
        }
        ...
        private void upHeapify(int c) {
            for (int p = (c - 1) / 2; c != 0 && distance[c] > distance[p]; c = p, p = (c - 1) / 2) {
                ...
                double[] pLoc = location[p];
                data[p] = data[c];
                distance[p] = distance[c];
                location[p] = location[c];
                data[c] = pData;
                distance[c] = pDist;
                location[c] = pLoc;
            }
