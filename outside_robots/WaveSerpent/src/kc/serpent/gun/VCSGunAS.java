...
public class VCSGunAS implements GunSystem {
    static final double NONREAL_WAVE_WEIGHT = 0.1;
    ...
    public class GunBuffer extends VCSAABuffer {
        ...
        public GunBuffer(int[] sliceIndexes, double bufferWeight, double[][][] metaSlices, double[][][][] metaIndexStats) {
            ...
            for (int i = 0; i < DIMENSIONS; i++) {
                totalBins *= (slices[i].length + 1);
            }

            rolling = Math.pow(0.97, Math.sqrt(totalBins));

            desiredWeight = 1 - rolling;
        }
    }
}
